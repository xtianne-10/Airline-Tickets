  ...
  fetch("/json/destinations.json")
    ...
    .then((data) => {
      ...
      const exploreDestinations = data.destinations.filter(
        (dest) => !excluded.includes(dest.name)
      );
      ...
      exploreDestinations.forEach((dest) => {
        ...
        const isActive = favorites.some((fav) => fav.name === dest.name)
          ? "active"
          : "";
		  ...
		  card.innerHTML = `
		    ...
		    <p class="explore_txt" data-country="${dest.country}">${dest.name}</p>
      ...
      // CARD CLICK HANDLER — autofill “To” input
      document.querySelectorAll(".explore_card").forEach((card) => {
        card.addEventListener("click", function (e) {
          if (e.target.closest(".star-icon")) return;

		  const destinationElem = card.querySelector(".explore_txt");
		  const destinationName = destinationElem.innerText;
		  const destinationCountry = destinationElem.dataset.country || "";
		  const toInput = document.getElementById("to");

		  if (toInput) {
		    toInput.value = `${destinationName}, ${destinationCountry}`;
		  } else {
